...
This project orchestrates multiple Singer data pipelines to extract data from various sources: Hubspot, Wrike, Xero and load it into a PostgreSQL database using a central scheduler.

## 1. How the Singer SDK Works (Conceptual Overview)
...
## 2. Deploy & Dev Instructions
### NOTE: the repo is ready to deploy on `Render Background Worker with Persistent Disk`
### Inputs:
*   **Build Command:** 
    ```bash
    $ poetry install
...
### For local testing set up:
#### *** The pipeline has only been tested on WSL***
1.  **Clone Repository:**
    ```bash
...
3.  **Configure Individual Pipelines:**
...
4.  **Install & Run the Project:**
    *   Run this single command line:
    ...
    - Initiates the full ETL + modeling sequence

    ### Each sequence performs:

    - Runs the appropriate pipelines 
    - Run the dbt logic after piplines
...
5.  **DEBUG:**
    ...
    *   Ensure these (potentially empty) files exist:
        ```bash
...
⛔ Press Ctrl+C in the terminal where `./run.sh` is running to stop the scheduler gracefully.  
